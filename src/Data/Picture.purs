...
data Shape
  ...
  | Line Point Point
  | Text Point String
  | Clipped Point Picture Number Number
...
showShape (Clipped loc pic w h) =
  "Picture [location: " <> showPoint loc <> ", width: " <> show w <> ", height: " <> show h <> "]"
...
shapeBounds (Clipped (Point { x, y }) _ w h ) = Bounds
    { top: y - h / 2.0
    , left: x - w / 2.0
    , bottom: y + h / 2.0
    , right: x + w / 2.0
